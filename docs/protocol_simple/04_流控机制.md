...
在深入 waitPeer 和 postPeer 之前，我们需要先理解 **Primitives 构造函数中如何设置 connStepPtr**。这是理解流控逻辑的关键。

在 Primitives 的 `loadRecvConn` 和 `loadSendConn` 函数中（[prims_simple.h:486-580](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/prims_simple.h#L486-L580)），有这样的设置：
