...
回忆第二章[数据结构详解](./02_数据结构详解.md)的关键概念：**tail 和 head 指针有方向性。**

- **tail**：
  - 发送方：存在**本地内存**，自己更新，对方读取
  - 接收方：存在**对方内存**，自己读取，对方更新

- **head**：
  - 接收方：存在**本地内存**，自己更新，对方读取
  - 发送方：存在**对方内存**，自己读取，对方更新

这个设计实现了"写本地，读远程"的模式，充分利用 GPU P2P 内存访问的能力。
...
if (flags & RoleWaitRecv) {
  ...
  connStepCache = loadStepValue(connStepPtr);  // 缓存远端 tail 值
  // ...
}
```
...
if (flags & RoleWaitSend) {
  ...
  connStepCache = loadStepValue(connStepPtr);  // 缓存远端 head 值
  // ...
}
```
...
对于发送方，`conn->tail` 指向**本地内存**，但接收方通过 P2P 可以读取。

**为什么用 relaxed？**

因为 fence 已经保证了顺序，这个 store 本身不需要额外的同步语义。接收方会用 volatile load 来读取（在 `loadStepValue` 中），保证看到数据写入。
...
对于接收方，`conn->head` 指向**本地内存**，发送方通过 P2P 可以读取。

当接收方更新 head 后，发送方在下次 waitPeer 时会看到这个更新，知道可以重用这个 slot 了。
...
|          | 设置 `dsts`（对方内存）                 | 设置 `srcs`（本地内存）               |
| 数据操作     | 写数据到对方内存                         | 读数据从本地内存                     |
| postPeer | **fence** + 更新 `tail`（本地内存）     | 更新 `head`（本地内存）              |
|          | 对方会轮询 `tail`                     | 对方会轮询 `head`                 |
