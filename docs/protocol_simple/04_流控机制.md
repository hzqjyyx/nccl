...
### 关键术语澄清：物理位置 vs 访问模式 vs 数据语义

在深入流控机制之前，我们必须先澄清一个容易混淆的概念。当我们说"本地"和"远端"时，到底指什么？

**理解流控机制需要区分三个维度：**

**1. 物理存储位置**：内存在哪个 GPU 上分配
...
- **Wait 角色（轮询）**：读取**本地 GPU 的内存**（快速访问）
  - 发送方 Wait 线程：读取本地的 head
  - 接收方 Wait 线程：读取本地的 tail

- **Post 角色（更新）**：写入**对方 GPU 的内存**（P2P 远程写入）
  - 发送方 Post 线程：通过 P2P 写入对方的 tail
  - 接收方 Post 线程：通过 P2P 写入对方的 head

**3. 数据语义**：计数器的值表示什么

- **head 的值**：表示接收方的读取进度（无论存储在哪）
- **tail 的值**：表示发送方的写入进度（无论存储在哪）

**这个设计的精妙之处**：

| 角色 | 操作 | 访问位置 | 访问速度 | 值的含义 |
|------|------|----------|----------|----------|
| 发送方 Wait | 读 head | 本地内存 | 快（本地访问） | 对方的读取进度 |
| 发送方 Post | 写 tail | 对方内存 | 慢（P2P 写入） | 自己的写入进度 |
| 接收方 Wait | 读 tail | 本地内存 | 快（本地访问） | 对方的写入进度 |
| 接收方 Post | 写 head | 对方内存 | 慢（P2P 写入） | 自己的读取进度 |

**关键洞察：这个"反直觉"的设计实现了"轮询本地，更新远端"的模式**：
- 高频的轮询操作访问本地内存（极快）
- 低频的更新操作通过 P2P 写入对方内存（可接受的开销）
- 虽然轮询的是本地内存，但值是对方写入的，反映对方的进度

**在后续章节中**，当我们说"读取 head"或"更新 tail"时，请记住这三个维度：物理位置、访问者、值的语义。
...
```c
// Worker 线程（比如 tid=50，总共 254 个 workers）
...
```
线程 0-1：   RoleWaitRecv/RoleWaitSend（轮询，设置指针）
线程 2-253： Worker 线程（并行拷贝和 reduce 数据）
线程 254-255：RolePostSend/RolePostRecv（更新计数器，fence）
```
