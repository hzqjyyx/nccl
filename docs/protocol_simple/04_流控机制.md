...
数轴（横轴是 step，注意不是 slot，step 是单调递增的）：
...
标注：
- head 到 tail 之间：已写入但还未读取的数据（slot 1-7）
- tail - head = 7（发送方领先 7 个 step）
- 对应的 slot：head % 8 = 1，tail % 8 = 0
- slot 0 是发送方下一个要写的位置
- slot 1 是接收方下一个要读的位置
</ImageDescription>
```
...
**核心关系**：
- `tail - head`：发送方领先接收方的步数，也就是"已写入但还未读取"的 slot 数量
- 如果 `tail - head = 0`：缓冲区为空，接收方要等
- 如果 `tail - head = 8`：缓冲区满了，发送方要等

### tail/head 存储设计回顾

回忆第二章[数据结构详解](./02_数据结构详解.md)的关键设计：**tail 和 head 的"反直觉"存储位置**。
...
在深入 waitPeer 和 postPeer 之前，我们需要先理解 **Primitives 构造函数中如何设置 connStepPtr** (Recap: [Primitives 操作的封装](./02_数据结构详解.md#24-primitives操作的封装))。这是理解流控逻辑的关键。

在 Primitives 的 `loadRecvConn` 函数中（[prims_simple.h:486-580](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/prims_simple.h#L486-L580)），有这样的设置：
...
if (flags & RolePostRecv) {
  connStepPtr = conn->head;  // 指向远端的 head（发送方的 SendMem），用于更新
  *connStepPtr = step;       // 初始化
}
if (flags & RoleWaitRecv) {
  connStepPtr = conn->tail;  // 指向本地的 tail（接收方的 RecvMem），用于轮询
  connStepCache = loadStepValue(connStepPtr);  // 缓存本地 tail 值
  // ...
}
```

**发送方的指针设置**
`loadSendConn` 也有类似的工作。

```c
// loadSendConn 函数中
if (flags & RolePostSend) {
  connStepPtr = conn->tail;  // 指向远端的 tail（接收方的 RecvMem），用于更新
}
if (flags & RoleWaitSend) {
  connStepPtr = conn->head;  // 指向本地的 head（发送方的 SendMem），用于轮询
  connStepCache = loadStepValue(connStepPtr);  // 缓存本地 head 值
  // ...
}
```

**关键洞察：**
- **RoleWaitRecv** 线程：`connStepPtr` 指向**本地的 tail**，`connStepCache` 缓存本地 tail 值（轮询本地）
- **RolePostRecv** 线程：`connStepPtr` 指向**远端的 head**，用于通过 P2P 更新
- **RoleWaitSend** 线程：`connStepPtr` 指向**本地的 head**，`connStepCache` 缓存本地 head 值（轮询本地）
- **RolePostSend** 线程：`connStepPtr` 指向**远端的 tail**，用于通过 P2P 更新

**记住这个规律**：Wait 角色轮询**本地**计数器（快），Post 角色更新**远端**计数器（P2P 写入）
...
while (connStepCache + (isSendNotRecv ? NCCL_STEPS : 0) < step + StepPerSlice) {
    connStepCache = loadStepValue(connStepPtr);  // 从远程读取计数器
}
```
...
while (connStepCache + NCCL_STEPS < step + StepPerSlice) {
    connStepCache = loadStepValue(connStepPtr);  // 从远程读取 head
}
```

结合前面讲的指针设置，对于发送方：
- `connStepPtr` 指向**远端的 head**
- `connStepCache` 缓存的是**远端的 head** 值
...
```c
while (远端head + 8 < 本地tail + 1) {
	// 等待接收方读走数据
    远端head = loadStepValue(远端head指针);
}

// **等价条件**（假设 StepPerSlice = 1）：
while (本地tail - 远端head >= 8) {
    // 等待
}
```
...
while (connStepCache + 0 < step + StepPerSlice) {
    connStepCache = loadStepValue(connStepPtr);  // 从远程读取 tail
}
```
...
结合指针设置，对于接收方：
- `connStepPtr` 指向**远端的 tail**
- `connStepCache` 缓存的是**远端的 tail** 值
- `step` 是**本地的 head**（接收方要读的位置）

所以这个条件的真实含义是：

```c
while (远端tail < 本地head + 1) {
    远端tail = loadStepValue(远端tail指针);
}
```
...
**这个条件的含义**：如果接收方的进度 >= 发送方的进度，说明没有新数据，要等待。
...
### `loadStepValue`：如何读取对端的计数器？

waitPeer 需要轮询对端的计数器（tail 或 head）。这是通过 `loadStepValue` 函数实现的。
...
**为什么用 volatile 或 atomic？**

因为这是一个轮询循环。如果不用 volatile，编译器可能会"优化"：

```c
// 错误的优化
uint64_t cached_value = *connStepPtr;
while (cached_value < step + 1) {
    // 死循环！因为 cached_value 不会更新
}
```

**volatile 或 atomic load 告诉编译器：这个值可能被外部修改（对方 GPU 写入），每次都要重新读取。**

在实际代码中（[prims_simple.h:85-105](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/prims_simple.h#L85-L105)），`loadStepValue` 使用 `ld_volatile_global` 实现：
...
我们在代码系列会详细分析。现在你只需要知道：**它从远程内存读取计数器的值，并保证不会被编译器优化掉。**
...
| 操作       | 发送方                              | 接收方                          |
|----------|----------------------------------|------------------------------|
| waitPeer | 等待 `connStepCache + NCCL_STEPS < step + StepPerSlice` | 等待 `connStepCache < step + StepPerSlice` |
|          | 轮询**本地** head，设置 `dsts`（对方内存）                 | 轮询**本地** tail，设置 `srcs`（本地内存）               |
| 数据操作     | 写数据到对方内存                         | 读数据从本地内存                     |
| postPeer | **fence** + 更新 `tail`（**远端**内存，P2P 写入）     | 更新 `head`（**远端**内存，P2P 写入）              |
|          | 对方会轮询**本地** tail                     | 对方会轮询**本地** head                 |

**关键的不对称**：

1. **等待条件不同**：发送方考虑环绕（+NCCL_STEPS），接收方只比较进度
2. **fence 只在发送方**：因为只有发送方写入数据，需要保证内存序
3. **指针方向相反**：发送方写对方内存，接收方读本地内存

这个不对称性是设计的精髓：**利用 P2P 内存访问的特性，让发送方直接写入对方内存，接收方从本地读取，最大化性能。**
...
在 `Primitives` 的构造函数中，线程被分配到四种角色之一（[prims_simple.h:619-624](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/prims_simple.h#L619-L624)）：
...
**关键点**：一个线程不会同时具有 Wait 和 Post 角色。

### 线程如何配合？

举个例子，假设一个 kernel 有 256 个线程，Ring AllReduce 中每个 GPU 有 1 个接收 peer 和 1 个发送 peer：

```
线程分配（简化）：
- 线程 0：RoleWaitRecv（负责从 GPU 3 接收的 wait）
- 线程 1：RoleWaitSend（负责向 GPU 1 发送的 wait）
- 线程 2-253：worker 线程（负责实际的数据拷贝）
- 线程 254：RolePostSend（负责向 GPU 1 发送的 post）
- 线程 255：RolePostRecv（负责从 GPU 3 接收的 post）
```
...
**在第六章（Primitives 构造与初始化）和第七章（waitPeer 代码剖析）**，我们会详细分析角色分配、线程同步的实现细节。现在你只需要知道：

**关键洞察：Wait 和 Post 由不同的线程执行，但它们在概念上形成一个完整的流控流程。线程分工让操作可以并行化，提高效率。**
...
写数据到 slot 1                                   | 检查：connStepCache < 1 + 1?
                                               | 1 < 2? Yes，要等待
**postPeer**                                    | 轮询 tail...
fence_acq_rel_sys()                            | 发现 tail = 2，1 < 2，可以读
...
**如果 GPU 1 还没读完 slot 0 怎么办？**

假设 GPU 1 很慢，到第 9 轮时，它才读到 slot 5：

```
tail = 8, head = 5
```

GPU 0 想写 slot 0（step = 8）：
...
但这是**不对的**！因为 GPU 1 的 head = 5，说明它只读到 step 5（slot 5），slot 0 对应的是 step 0，早就读完了。

等等，我搞混了。让我重新理清楚。

**head = 5 的含义**：GPU 1 已经读完 step 0, 1, 2, 3, 4，下一个要读的是 step 5。

**tail = 8 的含义**：GPU 0 已经写完 step 0, 1, 2, 3, 4, 5, 6, 7，下一个要写的是 step 8。

所以：
- step 0 对应 slot 0，GPU 1 已读完
- step 8 对应 slot 0（环绕），GPU 0 要写

这是安全的，因为 GPU 1 早就读完 step 0 了。

**危险的情况**：假设 GPU 1 非常慢，到第 16 轮时，它才读到 step 5：
...
GPU 0 会轮询 head，直到 `connStepCache + 8 >= 16 + 1`（即 `head >= 9`），才继续。
...
你可能注意到，waitPeer 使用的是**轮询（polling）**：不断读取对端的计数器，直到满足条件。
...
```c
// 只有当 cache 不满足条件时，才从远程读取
while (connStepCache + (isSendNotRecv ? NCCL_STEPS : 0) < step + StepPerSlice) {
    connStepCache = loadStepValue(connStepPtr);
}
```

这个 cache 减少了实际的远程内存访问次数。
...
- **RoleWaitRecv** 线程：`connStepPtr` 指向远端的 `tail`
- **RolePostRecv** 线程：`connStepPtr` 指向本地的 `head`
- **RoleWaitSend** 线程：`connStepPtr` 指向远端的 `head`
- **RolePostSend** 线程：`connStepPtr` 指向本地的 `tail`

这个设置决定了 waitPeer 和 postPeer 的行为。
...
- 含义：`while (远端head + 8 < 本地tail + 1)`
- 等价：`while (本地tail - 远端head >= 8)`
- 保证不会绕圈覆盖未读数据

**接收方**（`isSendNotRecv = false`）：
- 展开为：`while (connStepCache < step + StepPerSlice)`
- 含义：`while (远端tail < 本地head + 1)`
- 等价：`while (本地head >= 远端tail)`
- 保证只读取已写入的数据
...
- GPU 上轮询比中断更高效
- 等待时间短，不会浪费资源
- `connStepCache` 减少了远程内存访问

**关键洞察：流控机制的本质是用两个计数器和两个函数，实现了环形缓冲区的正确性和高性能。发送方的 fence 保证了内存序，waitPeer 的条件保证了不会覆盖旧数据，postPeer 的更新保证了双方的同步。理解 `connStepCache + (isSendNotRecv ? NCCL_STEPS : 0) < step + StepPerSlice` 这个统一的等待条件，以及它如何针对发送方和接收方展开为不同的逻辑，是理解流控机制的关键。**
