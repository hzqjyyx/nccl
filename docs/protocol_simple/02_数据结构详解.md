...
![memory](./excalidraws/02_memory_simple.excalidraw.png)

GPU 0 的 `buffs` 和 GPU 1 的 `buffs` 指向**同一块物理内存**：
- GPU 0 通过 P2P 直接写入 GPU 1 的内存
- **零拷贝**：没有中间缓冲，数据只传输一次

![memory](./excalidraws/02_memory_full.excalidraw.png)
...
Simple 协议的缓冲区大小与 step 大小在 Host 端初始化阶段统一计算，并通过 `ncclComm::buffSizes[]` 下发到设备端（见 `devCommSetup` 中的拷贝：`src/init.cc:525`）。本节给出默认值、环境变量以及运行时计算公式，便于在阅读/调参时有明确的数量级概念。

### 1. 环形缓冲区总大小（bytes）

- 定义：`buffSizes[NCCL_PROTO_SIMPLE]`
- 默认值：`DEFAULT_BUFFSIZE = 4 MiB`（[src/init.cc:699](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/init.cc#L699)）
- 可配置环境变量：`NCCL_BUFFSIZE`（[src/init.cc:700-702](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/init.cc#L700-L702)）
- 计算路径：在 `computeBuffSizes()` 中从环境变量读取，否则采用默认值（[src/init.cc:708-714](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/init.cc#L708-L714)）。

连接建立时，接收方为多个协议（LL/LL128/SIMPLE）在同一块设备内存上顺序切分区域：

```c
// 以 Recv 侧为例，按协议累加切分缓冲区（伪代码）
char* buff = (char*)(recvDevMem+1);
for (int p = 0; p < NCCL_NUM_PROTOCOLS; ++p) {
  recv->conn.buffs[p] = buff;
  buff += comm->buffSizes[p];  // 为下一协议挪出对应大小
}
```

对应实现可参考：`src/transport/p2p.cc:556-557` 与发送侧 `src/transport/p2p.cc:500-501`。

### 2. 每个 Step 的大小（bytes）

- Step 个数：`NCCL_STEPS = 8`（[src/include/device.h:24](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/include/device.h#L24)）
- 定义：`stepSizeBytes = buffSizes[NCCL_PROTO_SIMPLE] / NCCL_STEPS`
- 默认值：`4 MiB / 8 = 512 KiB`
- 赋值位置：连接建立时写入 `ncclConnInfo::stepSize`（[src/transport/p2p.cc:504](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/transport/p2p.cc#L504)、[src/transport/p2p.cc:547](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/transport/p2p.cc#L547)）
- 设备端读取：`ProtoSimple::calcBytePerStep()` 直接使用 `ncclShmem.comm.buffSizes[NCCL_PROTO_SIMPLE]/NCCL_STEPS`（[src/device/primitives.h:33-36](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/primitives.h#L33-L36)）

把字节数换算为“元素数”时，`connStepSize = stepSizeBytes / sizeof(T)`（设备端在 `loadRecvConn/loadSendConn` 中完成，参见 [src/device/prims_simple.h:506](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/prims_simple.h#L506)、[src/device/prims_simple.h:555](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/device/prims_simple.h#L555)）。

### 3. P2P Chunk 大小（与调度相关）

调度阶段还会使用一个“P2P Chunk 大小”（`comm->p2pChunkSize`），用于决定一次传输的分片大小（特别是网络路径）。其来源与约束如下：

- 默认值：
  - 多节点（`nNodes>1`）：`P2P_NET_CHUNKSIZE = 128 KiB`
  - 同机 NVLink 全连（`isAllNvlink`）：`P2P_NVL_CHUNKSIZE = 512 KiB`
  - 其他（PCIe）：`P2P_PCI_CHUNKSIZE = 128 KiB`
  - 定义与读取：见 [src/init.cc:704-718](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/init.cc#L704-L718)

- 约束（防止超过 Step）：
  - `if (p2pChunkSize * NCCL_STEPS > buffSizes[SIMPLE]) p2pChunkSize = buffSizes[SIMPLE]/NCCL_STEPS`（[src/init.cc:720-721](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/init.cc#L720-L721)）

- 在调度（enqueue）中的使用：
  - 对 SIMPLE 协议，调度时的 `stepSize` 取 `p2pChunkSize`（而非直接用 `buffSizes/8`），并据此派生 `chunkSize`（小消息时进一步减半/减四，网络友好）：见 [src/enqueue.cc:833-845](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/enqueue.cc#L833-L845)

小结：

```
// Host 初始化
buffSizes[SIMPLE] = NCCL_BUFFSIZE (若设置) else 4 MiB
stepSizeBytes      = buffSizes[SIMPLE] / 8       // 8 = NCCL_STEPS
p2pChunkSize       = {128 KiB | 512 KiB}         // 取决于拓扑
p2pChunkSize       = min(p2pChunkSize, stepSizeBytes)

// Device 侧（协议内核）
bytePerStep(Simple) = buffSizes[SIMPLE] / 8
connStepSize(Elts)  = bytePerStep / sizeof(T)
```
...
使用上
  - 发送端：`step = 下一次要“生产”的步号`；完成后把这个“新步号”写给对端 tail。
  - 接收端：`step = 下一次要“消费”的步号`；完成后把这个“新步号”写给对端 head。
  - 等待时
    - 发送端使用 `step` 与本地 `head` 比较（head 是接收端读取的值，反映对方的进度，防止追尾）
    - 接收端使用 `step` 与本地 `tail` 比较（tail 是发送端写入的值，反映自己的进度，等待到货）
