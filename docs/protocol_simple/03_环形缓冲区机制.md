...
- **第二章**：了解了关键数据结构（ncclConnInfo、ncclConnFifo、ncclSheme、ncclShmemGroup、Primitives），知道了"有什么"和"在哪里"
...
![](./excalidraws/03_buffer.excalidraw.svg)

**重要澄清**：tail 和 head 是**计数器（`uint64_t`）**，不是指针。它们的值是 step（单调递增的），通过 `% 8` 计算出对应的 slot 索引。tail 存储在接收方内存，head 存储在发送方内存，这个"反直觉"的设计让轮询操作（高频）访问本地内存，性能最优。详见[第二章：tail/head 存储设计](./02_数据结构详解.md#ncclconninfo连接的核心信息)。
...
NCCL 默认使用一个硬编码的值（定义在 [init.cc:699](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/init.cc#L699)）：
...
你也可以通过环境变量 `NCCL_BUFFSIZE` 手动指定（以字节为单位），但不能改变 slot 的数量（永远是 8）。
...
__device__ Primitives(...) {
  // 注意这里是 Primitives 的 stepSize，不是 ncclConnInfo.stepSize
  stepSize(stepSize_ == 0 ? ncclShmem.comm.buffSizes[NCCL_PROTO_SIMPLE]/NCCL_STEPS/sizeof(T) : stepSize_)
  // 假设默认 4MB 的 buffer，数据类型是 float（4 字节）
  // stepSize = 4MB / 8 / 4 = 128K 个 float
}
```
...
- `step = 1`：`slot = 1`，`slotAddr = 0x7f000000 + 1 * 1M * 4 = 0x7f400000`
...
但在某些场景下，一个 slice 可能跨越多个 step。[TODO: 这里需要补充一个具体的场景]
