...
**为什么需要清理？**
- flag 是 `uint32_t`，最大值 `0xffffffff`（42.9 亿），递增后会回绕到 0
- 环形缓冲区会重复使用同一个 slot：当 step 从 2^32 回绕到 0 时，slot 0 会被再次使用
- 如果不清理，slot 0 中可能残留着 step=0 时写入的 flag=1，当 step 回绕后再次变成 0 时，接收方会误认为"数据已就绪"（因为 flag 匹配），但实际读到的是 42.9 亿个 step 之前的旧数据

**何时触发清理？**
- 触发条件：`(sendStep[i] & NCCL_LL_CLEAN_MASK) == NCCL_LL_CLEAN_MASK`
- 在正常模式下，`NCCL_LL_CLEAN_MASK = 0x7ffffff8`（[src/include/device.h:95-103](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/include/device.h#L95-L103)）
  - 二进制：`0111 1111 1111 1111 1111 1111 1111 1000`（最低 3 位为 0）
  - 条件成立时：`sendStep[i]` 的最低 3 位可以是 0-7，高位必须全为 1
  - 因此触发窗口是：`sendStep[i] ∈ [0x7ffffff8, 0x7fffffff]`，即 **2147483640 到 2147483647 这 8 个连续 step**
  - 这是 `uint32_t` 回绕前的最后 8 个 step

**清理什么？**
- 遍历当前 step 的所有 line：`for (int o = offset; o<stepLines; o+=nthreads)`
- 对每个 line 调用：`storeLL(sendPtr(i)+o, 0, sendFlag(i))`
  - **数据写 0**（无关紧要，因为清理期间数据已经被正常的 storeLL 覆盖）
  - **标志写当前 flag**：这才是关键，用当前 step 的 flag 覆盖旧 flag
- 清理完成后，这 8 个 step 对应的 slot 中的所有 line 都带上了新的 flag，确保回绕后不会有旧 flag 残留

**为什么清理窗口是 8 个 step？**
- `NCCL_STEPS = 8`：环形缓冲区有 8 个 slot
- 清理窗口也是 8 个连续 step，确保每个 slot 都被清理一次
- 代码中有断言确保对齐：`static_assert(NCCL_LL_CLEAN_MASK % NCCL_STEPS == 0)`
- 如果不对齐，可能某个 slot 在清理窗口期间正在被使用（尚未读取完毕），提前清理会破坏数据

**测试模式**：
- 为便于测试，代码提供 `TEST_LL_CLEANUP` 模式（[src/include/device.h:96](https://github.com/NVIDIA/nccl/blob/v2.28.7-1/src/include/device.h#L96)）
- 在测试模式下，`NCCL_LL_CLEAN_MASK = 0x078`（清理窗口在 step 120-127）
- 这样无需等待 42.9 亿个 step，只需 127 步即可验证清理逻辑

**关键洞察**：
- flag 是 step 的"版本号"，通过 flag 匹配确保读到的是当前 step 的数据
- `step + 1` 的设计让 flag 从 1 开始（避免与初始化值 0 混淆）
- **清理机制是预防性的**：在回绕发生前提前清理，而不是回绕后补救
- 清理窗口与 `NCCL_STEPS` 对齐，确保每个 slot 恰好被清理一次
